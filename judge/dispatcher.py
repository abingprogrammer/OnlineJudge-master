...
from course.models import UserCourse,CourseSubmission
...
class CourseJudgeDispatcher(DispatcherBase):
    def __init__(self, submission_id, problem_id,course_id,task_id):
        super().__init__()
        self.submission = CourseSubmission.objects.get(id=submission_id)
    ...
    def update_problem_status(self):
        ...
        with transaction.atomic():
            ...
            # update_userprofile
            user_course = UserCourse.objects.select_for_update().get(user_id=self.submission.user_id,course_id=self.course_id)
            if problem.rule_type == ProblemRuleType.ACM:
                acm_problem_status = user_course.acm_problem_status
                if task_id not in acm_problem_status:
                    acm_problem_status[task_id] = {}
                    acm_problem_status[task_id][problem_id] = {"status": self.submission.result, "_id": self.problem._id}
                else:
                    if problem_id not in acm_problem_status[task_id]:
                        acm_problem_status[task_id][problem_id] = {"status": self.submission.result, "_id": self.problem._id}
                    elif acm_problem_status[task_id][problem_id]["status"] != JudgeStatus.ACCEPTED:
                        acm_problem_status[task_id][problem_id]["status"] = self.submission.result
                user_course.acm_problems_status = acm_problem_status
                user_course.save(update_fields=["acm_problem_status"])

            else:
                oi_problem_status = user_course.oi_problem_status
                if task_id not in oi_problem_status:
                    oi_problem_status[task_id] = {}
                    oi_problem_status[task_id][problem_id] = {"status": self.submission.result, "_id": self.problem._id}
                else:
                    if problem_id not in oi_problem_status[task_id]:
                        oi_problem_status[task_id][problem_id] = {"status": self.submission.result, "_id": self.problem._id}
                    elif oi_problem_status[task_id][problem_id]["status"] != JudgeStatus.ACCEPTED:
                        oi_problem_status[task_id][problem_id]["status"] = self.submission.result
                user_course.oi_problems_status = oi_problem_status
                user_course.save(update_fields=["oi_problem_status"])
